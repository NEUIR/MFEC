...
class Sari(evaluate.Metric):
    ...
    def _compute(self, sources, predictions, references):
        ...
        sari_add_score = 0
        for src, pred, refs in zip(sources, predictions, references):
            avgkeepscore,avgdelscore,avgaddscore,finalscore= SARIsent(normalize(src), normalize(pred), [normalize(sent) for sent in refs])
            sari_keep_score = sari_keep_score + avgkeepscore
            sari_del_score = sari_del_score + avgdelscore
            sari_add_score = sari_add_score + avgaddscore
            sari_score = sari_score + finalscore
        sari_score = sari_score / len(predictions)
        sari_keep_score = sari_keep_score / len(predictions)
        sari_del_score = sari_del_score / len(predictions)
        sari_add_score = sari_add_score / len(predictions)
        return {"keep": 100 * sari_keep_score,"del": 100 * sari_del_score,"add": 100 * sari_add_score,"sari": 100 * sari_score}