...
class RobertaForSequenceClassification(nn.Module):
    ...
    def forward(self, input_ids, input_mask):
        ...
        hidden_states_single = outputs_single[
            1]  # torch.tanh(self.hidden_layer_2(torch.tanh(self.hidden_layer_1(outputs_single[1])))) #(batch, hidden)
...
class EntailmentModel:
    def __init__(self, args):
        ...
        self.model = RobertaForSequenceClassification(num_labels,
                                                      entailment_tokenizer_path).cuda()  # .from_pretrained(pretrain_model_dir).cuda()
    ...
    def run_entailment_prediction(self, sample: Dict):
        ...
        sample['rouge_scores'] = []
        sample['corrections'] = sample['corrections'] + [sample['input_claim']]  # + sample['g_correction']
        for correction in sample['corrections']:
            ...
            for _, ctx in enumerate(evidence):
                ...
                input_ids = torch.LongTensor(
                    encoded_ctx_truncated + [self.tokenizer.sep_token_id] + encoded_correction).cuda().unsqueeze(0)
        ...
        if sample['correction_scores']:
            ...
            sample['final_answer'] = sample['corrections'][argmax]
        else:
